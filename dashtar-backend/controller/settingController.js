...
const addGlobalSetting = async (req, res) => {
  try {
    const adminId = req.body.adminId; // Extract adminId from the request
    const newGlobalSetting = new Setting({ ...req.body, adminId });
...
const getGlobalSetting = async (req, res) => {
  try {
    const adminId = req.query.adminId; // Extract adminId from the request
    const globalSetting = await Setting.findOne({ name: "globalSetting", adminId: adminId });
