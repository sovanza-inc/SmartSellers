...
const loginAdmin = async (req, res) => {
  try {
    ...
    if (admin && bcrypt.compareSync(req.body.password, admin.password)) {
      ...
      res.cookie('adminInfo', JSON.stringify(adminInfo), {
        httpOnly: true,  // Optional: Set true if you want the cookie to be accessible only by the web server
        secure: true,    // Optional: Set true if your site is HTTPS only
        maxAge: 3600000, // Cookie expiry time in milliseconds (e.g., 3600000 ms = 1 hour)
        sameSite: 'None' // Optional: Set to 'None', 'Lax', or 'Strict' as per your requirement
      });
